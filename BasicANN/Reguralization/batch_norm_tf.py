...
class HiddenLayerBatchNorm(object):
	...
	def forward(self, X, is_training, decay = 0.9):
		...
		if is_training:
			...
			updated_running_mean = tf.assign(self.running_mean, self.running_mean * decay + batch_mean * (1 - decay))
			updated_running_var = tf.assign(self.running_var, self.running_var * decay + batch_var * (1 - decay))

			with tf.control_dependencies([updated_running_mean, updated_running_var]):
